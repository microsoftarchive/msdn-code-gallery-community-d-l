...
Namespace ClientSample
	...
	Public Class FileIconManager
		...
		Public Shared Function GetFileIcon(ByVal name As String, ByVal size As IconSize, ByVal linkOverlay As Boolean) As Icon
		...
		Public Function AddFileIcon(ByVal filePath As String) As Integer
			...
			If _manageBothSizes Then
				...
				CType(_imageLists(0), ImageList).Images.Add(GetFileIcon(filePath, IconSize.Small, False))
				CType(_imageLists(1), ImageList).Images.Add(GetFileIcon(filePath, IconSize.Large, False))
			Else
				'only doing one size, so use IconSize as specified in _iconSize.
				CType(_imageLists(0), ImageList).Images.Add(GetFileIcon(filePath, _iconSize, False))
				'add to image list
			End If
