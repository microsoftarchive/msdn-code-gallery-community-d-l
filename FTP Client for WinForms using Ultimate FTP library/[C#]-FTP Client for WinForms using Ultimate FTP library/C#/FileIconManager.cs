    ...
    public class FileIconManager
    {
        ...
        public int AddFileIcon(string filePath)
        {
            ...
            if (_manageBothSizes)
            {
                ...
                ((ImageList) _imageLists[0]).Images.Add(GetFileIcon(filePath, IconSize.Small, false));
                ((ImageList) _imageLists[1]).Images.Add(GetFileIcon(filePath, IconSize.Large, false));
            }
            else
            {
                //only doing one size, so use IconSize as specified in _iconSize.
                ((ImageList) _imageLists[0]).Images.Add(GetFileIcon(filePath, _iconSize, false));
                //add to image list
            }
