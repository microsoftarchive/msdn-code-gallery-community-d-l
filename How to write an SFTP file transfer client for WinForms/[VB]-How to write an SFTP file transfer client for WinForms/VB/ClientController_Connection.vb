...
#If FTP AndAlso SFTP Then
			If serverType = 0 Then
				Dim forcedServerType As Integer = _loginInfo.Get(Of Integer)(LoginInfo.ServerType)

				If forcedServerType = 0 Then
					' Auto detect.
...
#If Not Framework4_5 Then
			...
			AddHandler _client.AuthenticateCompleted, AddressOf client_AuthenticateCompleted
			AddHandler _client.DownloadFilesCompleted, AddressOf client_DownloadFilesCompleted
			AddHandler _client.SynchronizeCompleted, AddressOf client_SynchronizeCompleted
...
#End If
		...
		Friend Function HandleException(ByVal exc As Exception, ByVal showError As Boolean) As Boolean
			...
			If _clientPlugin.ShouldReconnect(exc) Then
				Reconnect()
				Return False
			End If
		...
		''' </summary>
		Public Sub DoConnect(ByVal serverType As Integer)
...
#End If
		...
		Private _syncSavedRestoreState As Integer = -1
		''' <summary>
		''' Synchronizes local and remote folders.
		''' </summary>
		Public Sub DoSynchronize(ByVal remoteIsMaster As Boolean)
			EnableProgress(True)
...
#End If
		...
		Private Sub DoSynchronizePost(ByVal remoteIsMaster As Boolean)
			...
			If _syncSavedRestoreState <> -1 Then
...
#If Not Framework4_5 Then
		...
		Private Sub client_SynchronizeCompleted(ByVal sender As Object, ByVal e As AsyncCompletedEventArgs)
