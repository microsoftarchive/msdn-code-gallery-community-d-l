...
Namespace ClientSample
	...
	Partial Public Class Login
		...
		Public Sub New(ByVal s As SettingInfoBase)
			...
			cbxServerType.SelectedIndex = s.Get(Of Integer)(LoginInfo.ServerType)
...
#If FTP Then
			Dim enableAuth As Boolean = cbxProxyType.SelectedIndex <> CInt(Fix(FtpProxyType.Site)) AndAlso cbxProxyType.SelectedIndex <> CInt(Fix(FtpProxyType.Open)) ' User name and password are ignored with Site and FtpOpen proxy types.
#Else
			Dim enableAuth As Boolean = True
#End If

			cbxProxyMethod.Enabled = cbxProxyType.SelectedIndex = CInt(Fix(FtpProxyType.HttpConnect)) ' Authentication method is available for HTTP Connect only.
			txtProxyDomain.Enabled = cbxProxyMethod.Enabled AndAlso cbxProxyMethod.SelectedIndex = CInt(Fix(FtpProxyHttpConnectAuthMethod.Ntlm)) ' Domain is available for NTLM authentication method only.
